...
dfe['GGR perplexity'] = [model_dict[m]['GGR perplexity'] for m in dfe['model_name'].values]
...
for i, row in fpd_df.iterrows():
    ...
    dfe.loc[idx, 'FPD to GGR'] = row['protbert_fd_to_gigaref']

mets = ['UR50 perplexity', 'GGR perplexity', 'FPD to UR50', 'FPD to GGR']
for m in mets:
    fig, ax = plt.subplots(figsize=(4.8, 4.8))
