...
# if is_amlt:
#     cache_dir = os.path.join(os.environ["AMLT_OUTPUT_DIR"], "hf_cache")
#     os.makedirs(cache_dir, exist_ok=True, mode=0o777) #Give read and write permissions to all users
...
if __name__ == "__main__":
    ...
    # count all files in cache dir recursively with glob
    print("HF cache dir: ", cache_dir)
    print("Num cache dir files: ", len(glob(os.path.join(cache_dir, "**"), recursive=True)))
    ...
    if args.azure_storage_account_name is not None and args.azure_storage_container_name is not None:
        ...
        try:
            credentials = DefaultAzureCredential(logging_enable=True,exclude_managed_identity_credential=True)
